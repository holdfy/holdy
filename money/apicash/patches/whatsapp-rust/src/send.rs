...
impl Client {
    ...
    pub(crate) async fn send_message_impl(
        ...
        let stanza_to_send: wacore_binary::Node = if peer && !to.is_group() {
            ...
            // given (see Client::force_pn_addressing); status reactions share
            // this fanout but must keep LID addressing, so scoped to real DMs.
            ...
            let lock_jids = self.build_session_lock_keys(&all_dm_jids, force_pn_for_dm).await;

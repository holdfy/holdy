...
impl<'a> Contacts<'a> {
    ...
    // Vec<(Jid, Option<Jid>)> owned (não `impl IntoIterator<Item = (&'b Jid, ...)>` genérico):
    // a versão com lifetime genérica dispara um bug conhecido do rustc ("implementation of
    // FnOnce/Send/Iterator is not general enough") quando este future é aguardado dentro de
    // outro future spawnado (tokio::spawn) em crates downstream — reproduzido com rustc 1.95/1.96.
    ...
    pub async fn is_on_whatsapp(&self, jids: &[Jid]) -> Result<Vec<IsOnWhatsAppResult>> {
        ...
        self.persist_lid_mappings(
            results
                ...
                .filter_map(|r| {
                    if r.jid.is_lid() {
                        r.pn_jid.clone().map(|pn| (pn, Some(r.jid.clone())))
                    } else {
                        None
                    }
                })

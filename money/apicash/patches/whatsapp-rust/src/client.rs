...
pub struct Client {
    ...
    /// When true, outbound DM addressing (device fanout, session locks) keeps
    /// the PN namespace instead of upgrading to LID. Inbound decrypt addressing
    /// (`resolve_encryption_jid` elsewhere) is not affected. Hardcoded on:
    /// escape hatch for companion registrations whose LID-addressed DMs are
    /// silently 400-nacked by the server (upstream issue oxidezap/whatsapp-rust#941).
...
impl Client {
    /// See `Client::force_pn_addressing` field doc — hardcoded on (issue #941 workaround).
    pub(crate) fn force_pn_addressing_enabled(&self) -> bool {
    ...
    pub async fn new_with_cache_config(
        ...
        let this = Self {
            ...
            force_pn_addressing: AtomicBool::new(true),
            cache_config,

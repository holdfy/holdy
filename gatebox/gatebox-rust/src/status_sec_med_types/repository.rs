...
#[async_trait]
pub trait StatusSecMedTypesRepository: Send + Sync {
    async fn list(&self, offset: i64, limit: i64) -> Result<ItemsPage<Vec<StatusSecMedTypes>>, RepositoryError>;
    async fn get_by_id(&self, id: i64) -> Result<Option<StatusSecMedTypes>, RepositoryError>;
    async fn insert(&self, item: &StatusSecMedTypes) -> Result<i64, RepositoryError>;
    async fn update(&self, id: i64, item: &StatusSecMedTypes) -> Result<(), RepositoryError>;
    async fn delete(&self, id: i64) -> Result<bool, RepositoryError>;
}

pub struct StatusSecMedTypesRepositoryImpl {
    read: Arc<PgPool>,
    write: Arc<PgPool>,
}

impl StatusSecMedTypesRepositoryImpl {
...
#[async_trait]
impl StatusSecMedTypesRepository for StatusSecMedTypesRepositoryImpl {
    async fn list(&self, offset: i64, limit: i64) -> Result<ItemsPage<Vec<StatusSecMedTypes>>, RepositoryError> {
        let rows: Vec<Row> = sqlx::query_as(ddl::SQL_LIST).bind(limit).bind(offset).fetch_all(self.read.as_ref()).await?;
        let total = rows.len() as i64;
        let items = rows.into_iter().map(|r| StatusSecMedTypes {
            id: r.id,
            code: r.code,
    ...
    async fn get_by_id(&self, id: i64) -> Result<Option<StatusSecMedTypes>, RepositoryError> {
        ...
        Ok(row.map(|r| StatusSecMedTypes {
            id: r.id,
            code: r.code,
            description: r.description,
            full_count: None,

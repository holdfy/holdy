...
#[async_trait]
pub trait TypeAuthTypesRepository: Send + Sync {
    async fn list(&self, offset: i64, limit: i64) -> Result<ItemsPage<Vec<TypeAuthTypes>>, RepositoryError>;
    async fn get_by_id(&self, id: i64) -> Result<Option<TypeAuthTypes>, RepositoryError>;
    async fn insert(&self, item: &TypeAuthTypes) -> Result<i64, RepositoryError>;
    async fn update(&self, id: i64, item: &TypeAuthTypes) -> Result<(), RepositoryError>;
    async fn delete(&self, id: i64) -> Result<bool, RepositoryError>;
}

pub struct TypeAuthTypesRepositoryImpl {
    read: Arc<PgPool>,
    write: Arc<PgPool>,
}

impl TypeAuthTypesRepositoryImpl {
...
#[async_trait]
impl TypeAuthTypesRepository for TypeAuthTypesRepositoryImpl {
    async fn list(&self, offset: i64, limit: i64) -> Result<ItemsPage<Vec<TypeAuthTypes>>, RepositoryError> {
        let rows: Vec<Row> = sqlx::query_as(ddl::SQL_LIST).bind(limit).bind(offset).fetch_all(self.read.as_ref()).await?;
        let total = rows.len() as i64;
        let items = rows.into_iter().map(|r| TypeAuthTypes {
            id: r.id,
            code: r.code,

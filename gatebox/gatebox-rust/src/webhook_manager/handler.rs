...
use super::service::{WebhookManagerService, ServiceError};

#[derive(Clone)]
pub struct AppState {
    pub service: Arc<dyn WebhookManagerService>,
}
...
pub fn routes(service: Arc<dyn WebhookManagerService>) -> Router {
    Router::new()
...
) -> Result<Json<ItemsPage<Vec<WebhookManager>>>, AppError> {
...
async fn get_by_id(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> Result<Json<Option<WebhookManager>>, AppError> {

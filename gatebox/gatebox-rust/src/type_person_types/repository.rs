...
#[async_trait]
pub trait TypePersonTypesRepository: Send + Sync {
    async fn list(&self, offset: i64, limit: i64) -> Result<ItemsPage<Vec<TypePersonTypes>>, RepositoryError>;
    async fn get_by_id(&self, id: i64) -> Result<Option<TypePersonTypes>, RepositoryError>;
    async fn insert(&self, item: &TypePersonTypes) -> Result<i64, RepositoryError>;
    async fn update(&self, id: i64, item: &TypePersonTypes) -> Result<(), RepositoryError>;
    async fn delete(&self, id: i64) -> Result<bool, RepositoryError>;
}

pub struct TypePersonTypesRepositoryImpl {
    read: Arc<PgPool>,
    write: Arc<PgPool>,
}

impl TypePersonTypesRepositoryImpl {
...
#[async_trait]
impl TypePersonTypesRepository for TypePersonTypesRepositoryImpl {
    async fn list(&self, offset: i64, limit: i64) -> Result<ItemsPage<Vec<TypePersonTypes>>, RepositoryError> {
        let rows: Vec<Row> = sqlx::query_as(ddl::SQL_LIST).bind(limit).bind(offset).fetch_all(self.read.as_ref()).await?;
        let total = rows.len() as i64;
        let items = rows.into_iter().map(|r| TypePersonTypes {
            id: r.id,
            code: r.code,

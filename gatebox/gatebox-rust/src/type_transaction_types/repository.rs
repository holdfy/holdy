...
#[async_trait]
pub trait TypeTransactionTypesRepository: Send + Sync {
    async fn list(&self, offset: i64, limit: i64) -> Result<ItemsPage<Vec<TypeTransactionTypes>>, RepositoryError>;
    async fn get_by_id(&self, id: i64) -> Result<Option<TypeTransactionTypes>, RepositoryError>;
    async fn insert(&self, item: &TypeTransactionTypes) -> Result<i64, RepositoryError>;
    async fn update(&self, id: i64, item: &TypeTransactionTypes) -> Result<(), RepositoryError>;
    async fn delete(&self, id: i64) -> Result<bool, RepositoryError>;
}

pub struct TypeTransactionTypesRepositoryImpl {
    read: Arc<PgPool>,
    write: Arc<PgPool>,
}

impl TypeTransactionTypesRepositoryImpl {
...
#[async_trait]
impl TypeTransactionTypesRepository for TypeTransactionTypesRepositoryImpl {
    async fn list(&self, offset: i64, limit: i64) -> Result<ItemsPage<Vec<TypeTransactionTypes>>, RepositoryError> {
        let rows: Vec<Row> = sqlx::query_as(ddl::SQL_LIST).bind(limit).bind(offset).fetch_all(self.read.as_ref()).await?;
        let total = rows.len() as i64;
        let items = rows.into_iter().map(|r| TypeTransactionTypes {
            id: r.id,
            code: r.code,
    ...
    async fn get_by_id(&self, id: i64) -> Result<Option<TypeTransactionTypes>, RepositoryError> {
        ...
        Ok(row.map(|r| TypeTransactionTypes {
            id: r.id,
            code: r.code,
            description: r.description,
            full_count: None,

...
#[async_trait]
pub trait AccountStatusTypesRepository: Send + Sync {
    async fn list(&self, offset: i64, limit: i64) -> Result<ItemsPage<Vec<AccountStatusTypes>>, RepositoryError>;
    async fn get_by_id(&self, id: i64) -> Result<Option<AccountStatusTypes>, RepositoryError>;
    async fn insert(&self, item: &AccountStatusTypes) -> Result<i64, RepositoryError>;
    async fn update(&self, id: i64, item: &AccountStatusTypes) -> Result<(), RepositoryError>;
    async fn delete(&self, id: i64) -> Result<bool, RepositoryError>;
}

pub struct AccountStatusTypesRepositoryImpl {
    read: Arc<PgPool>,
    write: Arc<PgPool>,
}

impl AccountStatusTypesRepositoryImpl {
...
#[async_trait]
impl AccountStatusTypesRepository for AccountStatusTypesRepositoryImpl {
    async fn list(&self, offset: i64, limit: i64) -> Result<ItemsPage<Vec<AccountStatusTypes>>, RepositoryError> {
        let rows: Vec<Row> = sqlx::query_as(ddl::SQL_LIST).bind(limit).bind(offset).fetch_all(self.read.as_ref()).await?;
        let total = rows.len() as i64;
        let items = rows.into_iter().map(|r| AccountStatusTypes {
            id: r.id,
            code: r.code,
    ...
    async fn get_by_id(&self, id: i64) -> Result<Option<AccountStatusTypes>, RepositoryError> {
        ...
        Ok(row.map(|r| AccountStatusTypes {
            id: r.id,
            code: r.code,
            description: r.description,
            full_count: None,

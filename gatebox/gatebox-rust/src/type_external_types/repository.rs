...
#[async_trait]
pub trait TypeExternalTypesRepository: Send + Sync {
    async fn list(&self, offset: i64, limit: i64) -> Result<ItemsPage<Vec<TypeExternalTypes>>, RepositoryError>;
    async fn get_by_id(&self, id: i64) -> Result<Option<TypeExternalTypes>, RepositoryError>;
    async fn insert(&self, item: &TypeExternalTypes) -> Result<i64, RepositoryError>;
    async fn update(&self, id: i64, item: &TypeExternalTypes) -> Result<(), RepositoryError>;
    async fn delete(&self, id: i64) -> Result<bool, RepositoryError>;
}

pub struct TypeExternalTypesRepositoryImpl {
    read: Arc<PgPool>,
    write: Arc<PgPool>,
}

impl TypeExternalTypesRepositoryImpl {
...
#[async_trait]
impl TypeExternalTypesRepository for TypeExternalTypesRepositoryImpl {
    async fn list(&self, offset: i64, limit: i64) -> Result<ItemsPage<Vec<TypeExternalTypes>>, RepositoryError> {
        let rows: Vec<Row> = sqlx::query_as(ddl::SQL_LIST).bind(limit).bind(offset).fetch_all(self.read.as_ref()).await?;
        let total = rows.len() as i64;
        let items = rows.into_iter().map(|r| TypeExternalTypes {
            id: r.id,
            code: r.code,
    ...
    async fn get_by_id(&self, id: i64) -> Result<Option<TypeExternalTypes>, RepositoryError> {
        ...
        Ok(row.map(|r| TypeExternalTypes {
            id: r.id,
            code: r.code,
            description: r.description,
            full_count: None,

...
#[async_trait]
pub trait TypeAuthorizeTypesRepository: Send + Sync {
    async fn list(&self, offset: i64, limit: i64) -> Result<ItemsPage<Vec<TypeAuthorizeTypes>>, RepositoryError>;
    async fn get_by_id(&self, id: i64) -> Result<Option<TypeAuthorizeTypes>, RepositoryError>;
    async fn insert(&self, item: &TypeAuthorizeTypes) -> Result<i64, RepositoryError>;
    async fn update(&self, id: i64, item: &TypeAuthorizeTypes) -> Result<(), RepositoryError>;
    async fn delete(&self, id: i64) -> Result<bool, RepositoryError>;
}

pub struct TypeAuthorizeTypesRepositoryImpl {
    read: Arc<PgPool>,
    write: Arc<PgPool>,
}

impl TypeAuthorizeTypesRepositoryImpl {
...
#[async_trait]
impl TypeAuthorizeTypesRepository for TypeAuthorizeTypesRepositoryImpl {
    async fn list(&self, offset: i64, limit: i64) -> Result<ItemsPage<Vec<TypeAuthorizeTypes>>, RepositoryError> {
        let rows: Vec<Row> = sqlx::query_as(ddl::SQL_LIST).bind(limit).bind(offset).fetch_all(self.read.as_ref()).await?;
        let total = rows.len() as i64;
        let items = rows.into_iter().map(|r| TypeAuthorizeTypes {
            id: r.id,
            code: r.code,
    ...
    async fn get_by_id(&self, id: i64) -> Result<Option<TypeAuthorizeTypes>, RepositoryError> {
        ...
        Ok(row.map(|r| TypeAuthorizeTypes {
            id: r.id,
            code: r.code,
            description: r.description,
            full_count: None,

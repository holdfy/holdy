...
use super::service::{FeesService, ServiceError};

#[derive(Clone)]
pub struct AppState {
    pub service: Arc<dyn FeesService>,
}
...
pub fn routes(service: Arc<dyn FeesService>) -> Router {
    Router::new()
...
async fn list(State(state): State<AppState>, Query(p): Query<PaginationQuery>) -> Result<Json<ItemsPage<Vec<Fees>>>, AppError> {
    let limit = p.limit.unwrap_or(10).clamp(1, 100);
    let offset = p.offset.unwrap_or(0).max(0);
    Ok(Json(state.service.list(offset, limit).await?))
}
async fn get_by_id(State(state): State<AppState>, Path(id): Path<String>) -> Result<Json<Option<Fees>>, AppError> {
    let id: i64 = id.parse().map_err(|_| AppError::BadRequest("Invalid id"))?;
    Ok(Json(state.service.get_by_id(id).await?))
}
async fn create(State(state): State<AppState>, Json(item): Json<Fees>) -> Result<Json<serde_json::Value>, AppError> {
    let id = state.service.create(&item).await?;
    Ok(Json(serde_json::json!({ "id": id })))
}
async fn update(State(state): State<AppState>, Path(id): Path<String>, Json(item): Json<Fees>) -> Result<StatusCode, AppError> {
    let id: i64 = id.parse().map_err(|_| AppError::BadRequest("Invalid id"))?;

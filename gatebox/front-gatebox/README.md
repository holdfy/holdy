...
Projeto front-end do Gatebox, baseado em **Material Dashboard 3 PRO React**. Consome a API **gatebox-rust** (ou gateboxgo).  
**Plano único (telas, APIs por módulo):** [PLANO_FRONT_GATEBOX.md](./PLANO_FRONT_GATEBOX.md)  
**Checklist detalhado de telas:** [CHECKLIST_TELAS.md](./CHECKLIST_TELAS.md)
...
# 2. Configurar API (crie .env a partir de .env.example)
...
O frontend roda em `http://localhost:3000` por padrão. A API gatebox-rust usa porta 8080 por padrão (ou 8081 se configurado; ajuste `REACT_APP_API_BASE_URL` no `.env`).
